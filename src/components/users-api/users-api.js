...
export default class UsersAPIComponent extends Component {
    ...
    componentDidMount() {
        const {usersPage: {users, pageSize, currentPage}} = this.props;
        if(users.length === 0) {
    ...
    render() {
        const {usersPage: {users, currentPage, totalUsersCount, pageSize, loading, followingInProgress}, follow, unFollow} = this.props;
